...
class StockMovement(Base):
    ...
    # Identifiant de lot : tous les mouvements d'une même opération partagent
...
class StockBatch(Base):
    """Un lot d'entrée de stock — utilisé pour le calcul FIFO du prix d'achat.

    Chaque achat fournisseur (ou toute entrée de stock) crée un lot avec
    le prix unitaire d'achat. Lors d'une vente, les lots les plus anciens
    sont consommés en premier (FIFO) : quantity_remaining diminue jusqu'à 0.
    """
    ...
    def __repr__(self) -> str:
        return (
            f"<StockBatch display_id={self.display_id} "
            f"remaining={self.quantity_remaining}/{self.quantity_original}>"
        )
